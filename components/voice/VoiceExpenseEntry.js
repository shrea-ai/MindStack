...
export default function VoiceExpenseEntry({ onExpenseAdded, onClose }) {
  ...
  const processVoiceInput = async (voiceText, alternatives = []) => {
    ...
    try {
      ...
      setError('Voice processing service is not available. Please use manual entry or check if GEMINI_API_KEY is configured.')
    } finally {
      setIsProcessing(false)
    }
  }
  ...
  const confirmExpense = async () => {
    ...
    try {
      const payload = {
        ...
        type: processedExpense?.type || 'debit', // Include transaction type
        entryMethod: 'voice',
          ...
          <div className="space-y-4">
            ...
            <div className="flex justify-center py-3">
              <button
                ...
                className={`relative w-20 h-20 rounded-full flex items-center justify-center transition-all duration-300 shadow-lg ${isListening
          ...
          <div className="space-y-4">
            ...
            <div className="flex items-center justify-between">
              ...
              <button
                ...
                className={`flex items-center gap-1 px-3 py-1.5 rounded-lg text-sm font-medium transition-colors ${isEditing
            ...
            <div className={`rounded-xl p-4 border-2 ${isEditing ? 'bg-blue-50 border-blue-200' : 'bg-emerald-50 border-emerald-200'
              }`}>
              {/* Confidence Badge */}
              {!isEditing && (
                <div className="flex justify-end mb-2">
                  <span className={`text-xs px-2 py-0.5 rounded-full ${confidence >= 0.8 ? 'bg-green-100 text-green-700' :
              ...
              <div className="flex justify-center mb-3">
                {processedExpense.type === 'credit' ? (
                  <div className="flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-green-50 to-emerald-50 border-2 border-green-200 rounded-full">
                    <span className="w-3 h-3 bg-green-500 rounded-full animate-pulse"></span>
                    <span className="font-bold text-green-700">💰 Money In (Credit)</span>
                  </div>
                ) : (
                  <div className="flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-red-50 to-rose-50 border-2 border-red-200 rounded-full">
                    <span className="w-3 h-3 bg-red-500 rounded-full animate-pulse"></span>
                    <span className="font-bold text-red-700">💸 Money Out (Debit)</span>
                  </div>
                )}
              </div>
              ...
              <div className="space-y-3">
                ...
                <div className="flex items-center justify-between">
                  ...
                  {isEditing ? (
                    ...
                    <span className={`font-bold text-xl ${processedExpense.type === 'credit' ? 'text-green-600' : 'text-red-600'
                      }`}>
                      {processedExpense.type === 'credit' ? '+' : '-'}₹{processedExpense.amount}
                    </span>
                  )}
                </div>
                        ...
                        <div className="absolute right-0 mt-1 w-48 bg-white border rounded-xl shadow-lg z-10 py-1 max-h-60 overflow-y-auto">
                          {CATEGORY_OPTIONS.map((cat) => (
                            <button
                              ...
                              className={`w-full flex items-center gap-2 px-3 py-2 hover:bg-gray-100 transition-colors ${editCategory === cat.id ? 'bg-emerald-50 text-emerald-700' : ''
                                }`}
                            >
                              <span>{cat.emoji}</span>
            ...
            <div className="flex gap-3">
              <button
                ...
                disabled={isSaving}
                className={`flex-1 py-3 px-4 rounded-xl font-medium flex items-center justify-center text-white transition-all ${isSaving
