...
export class VoiceExpenseProcessor {
  constructor() {
    ...
    this.financialTerms = {
      ...
      // Transaction type keywords (CREDIT vs DEBIT)
      transactionTypes: {
        credit: [
          // English
          'added', 'received', 'credited', 'got', 'income', 'earned', 'deposited',
          'salary', 'bonus', 'refund', 'refunded', 'cashback', 'gained',
          // Hindi
          'मिला', 'मिले', 'आया', 'प्राप्त', 'जमा', 'वेतन', 'सैलरी'
        ],
        debit: [
          // English
          'debited', 'given', 'given to', 'paid', 'spent', 'deducted', 'transferred',
          'deleted', 'bought', 'purchased', 'withdrew', 'withdrawal', 'lost',
          'expense', 'खर्च', 'sent',
          // Hindi
          'दिया', 'दिए', 'खर्च', 'खरीदा', 'भुगतान'
        ]
      },
  ...
  async processVoiceInput(voiceText) {
    try {
      ...
      console.log('📝 Voice text length:', voiceText.length)
      console.log('🔤 Voice text (raw):', JSON.stringify(voiceText))

      // First try rule-based extraction for common patterns
      const ruleBasedResult = this.extractWithRules(voiceText)
      if (ruleBasedResult.confidence > 0.8) {
        console.log('✅ Rule-based extraction succeeded with confidence:', ruleBasedResult.confidence)
        console.log('📊 Extracted data:', ruleBasedResult.data)
  ...
  extractWithRules(text) {
    ...
    const transactionType = this.detectTransactionType(normalizedText)
...
TRANSACTION TYPE DETECTION (VERY IMPORTANT):
**CREDIT (Money In)** - Keywords:
- English: received, credited, added, got, income, earned, deposited, salary, bonus, refund
- Hindi: मिला, मिले, आया, प्राप्त, जमा, वेतन, सैलरी
- Hinglish: mila, mile, aaya, mil gaya, receive
- Examples: "200 मिले", "salary received", "मिला 1000", "aaya 500"

**DEBIT (Money Out)** - Keywords:
- English: spent, paid, given, deducted, bought, purchased, transferred
- Hindi: दिया, खर्च, खरीदा, भुगतान, काटा
- Hinglish: diya, kharch, kharcha, de diya, kharid liya
- Examples: "200 diya", "खर्च 500", "spent on food", "kharida 1000"
...
✓ "200 मिले" → {"amount": 200, "type": "credit", "category": "other", "description": "Money received"}
✓ "मुझे 1000 रुपये मिले" → {"amount": 1000, "type": "credit", "category": "other", "description": "Money received"}
✓ "salary received 5000" → {"amount": 5000, "type": "credit", "category": "other", "description": "Salary"}
✓ "200 रुपये दिए" → {"amount": 200, "type": "debit", "category": "other", "description": "Money given"}
✓ "bought new shoes of 200 rupees" → {"amount": 200, "type": "debit", "category": "shopping", "merchant": null, "description": "New shoes", "confidence": 0.95}
✓ "spent 500 on petrol" → {"amount": 500, "type": "debit", "category": "transport", "merchant": null, "description": "Petrol", "confidence": 0.95}
✓ "200 ka dosa khaya" → {"amount": 200, "type": "debit", "category": "food", "merchant": null, "description": "Dosa", "confidence": 0.95}
✓ "Metro में 45 spend kiya" → {"amount": 45, "type": "debit", "category": "transport", "merchant": "Metro", "description": "Metro travel", "confidence": 0.9}
✓ "Swiggy से biryani order 350 ka" → {"amount": 350, "type": "debit", "category": "food", "merchant": "Swiggy", "description": "Biryani from Swiggy", "confidence": 0.95}
...
Return ONLY valid JSON (no markdown code blocks, no extra text):
{
  ...
  "type": "credit" | "debit" (detect from keywords: received/added/credited/mila = credit, spent/paid/given/diya/kharch = debit),
...
`
      ...
      return {
        ...
        data: {
          ...
          type: extracted.type || 'debit', // Default to debit if not specified
          originalText: text,
          method: 'ai-powered'
        }
      }
  ...
  // Detect transaction type (credit vs debit) based on keywords
  detectTransactionType(text) {
    const normalizedText = text.toLowerCase()

    console.log('🔍 Detecting transaction type for:', text)

    // Credit keywords - comprehensive list with Hinglish
    const creditKeywords = [
      // English
      'added', 'received', 'credited', 'got', 'income', 'earned', 'deposited',
      'salary', 'bonus', 'refund', 'refunded', 'cashback', 'gained', 'receive',
      'credit', 'deposit', 'add',

      // Hinglish (phonetic)
      'aaya', 'aya', 'mila', 'mile', 'mil gaya', 'mil gaye', 'aa gaya', 'aa gaye',
      'receive', 'mili', 'miley', 'aai', 'prapt',

      // Hindi full words
      'मिला', 'मिले', 'मिली', 'मिलीं', 'आया', 'आए', 'आई', 'आईं',
      'प्राप्त', 'जमा', 'वेतन', 'सैलरी', 'बोनस'
    ]
    ...
    const debitKeywords = [
      ...
      'debited', 'given', 'given to', 'paid', 'spent', 'deducted', 'transferred',
      'deleted', 'bought', 'purchased', 'withdrew', 'withdrawal', 'lost',
      'expense', 'sent', 'pay', 'spend', 'buy', 'purchase',

      // Hinglish (phonetic)
      'diya', 'diye', 'diye', 'kharch', 'kharcha', 'de diya', 'de diye',
      'kata', 'kat gaya', 'nikala', 'nikal gaya', 'kharida', 'kharid liya',

      // Hindi full words
      'दिया', 'दिए', 'दी', 'दीं', 'खर्च', 'खर्चा', 'खरीदा', 'खरीदे',
      'काटा', 'निकाला', 'भुगतान', 'दे दिया', 'दे दिए'
    ]

    // Check for credit keywords (exact match)
    for (const keyword of creditKeywords) {
      if (normalizedText.includes(keyword)) {
        console.log('✅ Credit detected via keyword:', keyword)
        return 'credit'
      }
    }

    // Check for debit keywords (exact match)
    for (const keyword of debitKeywords) {
      if (normalizedText.includes(keyword)) {
        console.log('❌ Debit detected via keyword:', keyword)
        return 'debit'
      }
    }

    // Partial matching for Hindi root words (more flexible)
    // This handles variations like मिला, मिले, मिल गया, etc.
    const creditRoots = ['मिल', 'आ', 'प्राप्', 'जमा']
    const debitRoots = ['दि', 'खर्', 'काट', 'निकाल', 'खरीद']

    for (const root of creditRoots) {
      if (normalizedText.includes(root)) {
        console.log('✅ Credit detected via Hindi root:', root)
        return 'credit'
      }
    }

    for (const root of debitRoots) {
      if (normalizedText.includes(root)) {
        console.log('❌ Debit detected via Hindi root:', root)
        return 'debit'
      }
    }

    // Default to debit (money out) if no keyword found
    // This is safer as most voice entries are expenses
    console.log('⚠️ No keyword detected, defaulting to debit')
    return 'debit'
  }
